...
const BASE_CONFIG = {
  mainnet: {
    usdc: "0x036CbD53842c5426634e7929541eC2318f3dCF7e", // USDC on Base mainnet (corrected address)
...
async function main() {
  ...
  const deploymentInfo: DeploymentInfo = {
    ...
    deployedSlotMachines: [slotMachineAddress],
...
NEXT_PUBLIC_SLOT_MACHINE_ADDRESS=${slotMachineAddress}
...
`;
  ...
  const verifyScript = `// SPDX-License-Identifier: MIT
// Verification script for MainSlotManager
// Run with: npx hardhat verify --network ${networkName} ${managerAddress} ${constructorArgs.map(arg => \`"\${arg}"\`).join(' ')}

// Manager verification:
// npx hardhat verify --network ${networkName} ${managerAddress} ${constructorArgs.map(arg => \`"\${arg}"\`).join(' ')}

// Slot machine verification (implementation):
// npx hardhat verify --network ${networkName} ${slotMachineImplementation}

// Slot machine clone verification:
// npx hardhat verify --network ${networkName} ${slotMachineAddress}
`;

  fs.writeFileSync('./scripts/verify-manager.ts', verifyScript);
  console.log("📝 Verification script created (scripts/verify-manager.ts)");
  ...
  console.log(`🔗 View Slot Machine: https://${networkName.includes("sepolia") ? "sepolia." : ""}basescan.org/address/${slotMachineAddress}`);
